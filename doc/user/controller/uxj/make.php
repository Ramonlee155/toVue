...
switch ($_REQUEST['xtype']) {
    ...
    case 'upl':
        ...
        $m1 = $_POST['m1'];
        $time = sqltime(time());
        $msql->query("select * from `{$tb_kj}` where gid='{$gid}' and m1!='' and closetime<'{$time}' order by gid,closetime desc limit 1");
        $msql->next_record();
        ...
        //if ($fenlei != 100) {
            ...
            $tpl->assign('tu', $tu);
            if ($fenlei != 151 & 1 == 2) {
                $kj = getkjs($gid, $config['mnum'], 30, sqltime(time()),$fenlei);
                $tpl->assign('kj', $kj);
                $co[0] = 'blue';
                $co[1] = 'orange';
                $co[2] = 'lv';
...
function getbuzlong()
{
    ...
    $i = 0;
    //echo "select * from `{".$tb_play."}` where gid='{".$gid."}' and buzqishu>=2 and cid in (select cid from `{".$tb_class."}` where gid='{".$gid."}' and ftype=0)   and name not in('质','合','总尾质','总尾合','和尾质','和尾合') order by buzqishu desc,bid,sid,cid,xsort";
    $tmp = array();
    while ($fsql->next_record()) {
        $pname = $fsql->f('name');
        if ($tmp['b' . $fsql->f('bid')] == '') {
