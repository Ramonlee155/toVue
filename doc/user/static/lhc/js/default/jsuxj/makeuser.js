...
function myready() {
	...
	$(".zje").keyup(function () {
		...
		$(window.parent.document).find(".zje").val(val);
	}).keypress(function () {
		$(this).val($(this).val().replace(/\D/g, ''));
		var val = $(this).val()
		$(".zje").val(val);
...
function onefunc() {
	...
	$(".sub div a", parent.document).click(function () {
		...
		if (fenlei == 100) {
			// Enly
			$(window.parent.document).find("#side .user_info").show();
			$(window.parent.document).find("#side .betdone").show();
			$(window.parent.document).find("#quick_sec_table").hide();
			...
			if (bname == '特碼' | bname == '正特' | bname == '正碼') {
				...
				$('.lottery_info_left').show();
				$('.quick_select').show();
				if (fastinput == 1) { $(".fasttext").show(); }
			} else {
				$(".fasttext").hide();
				$("a.ab").hide();
				$('.lottery_info_left').hide();
				$('.quick_select').hide();
			}
		} else {
			$("a.ab").hide();
	...
	$(".cancel").click(function () {
		...
		$(".ggpeilv").html('');
		$(".textnr").val('');
		$(".quick_table input[name=\"bigType\"]").attr('checked',false);
					...
					if (m[4][0] != '' & m[4][0] != null) {
						...
						for (i = 0; i < ml; i++) {
							...
							if (fenlei == 163) {
								...
								if(Number(m[4][i]) <10)  m[4][i] = '0'+Number(m[4][i]) ;
...
function getlast15() {
	$.ajax({
		...
		success: function (m) {
			...
			//判断六合彩
			if($(".issixLhcgid", parent.document).val() == 1){
				var obj = $(".last15 .list", parent.document);
				var mount = 0 ;
				for (i = 0; i < ml; i++) {
					peilv = m[i]['peilv1'];
					...
					mount += Number(m[i]['je'] );
					//单注号不显示  m[i]['tid'] + "#"
					str += " <tr><td>" + m[i]['time'] + "</td><td>" + m[i]['je'] + "</td><td>" + m[i]['name'] + "@" + peilv + "</td></tr>";
				...
				str += "<tr><td class='ml' ml='"+ml+"' colSpan=\"3\" style=\"text-align: right; padding-right:10px\">共"+ml+"注,合计: "+mount+" </td></tr>";
				if(ml>0){
					$("#lastBets", parent.document).show();
					$(".user_info", parent.document).show();
					$("#betsBox", parent.document).hide();
				}
			}else{
	...
	$(".lists td.p").click(function () {
		...
		if ($(this).hasClass('selected')) {
			...
			if (!$(".lists td.i" + pid).find("input:text").prop("disabled")) {
				$(".lists td.i" + pid).find("input:text").val("");
			}
		} else {
