			...
			$(".user_tb a.record").click(function() {
				...
				$.ajax({
					...
					success: function(m) {
						...
						var estr = "<thead><tr><th>修改时间</th><th>修改者</th><th>修改IP</th><th>来源</th><th>变更前</th><th>变更后</th><th>备注</th></tr></thead><tbody>";
						for (i = 0; i < el; i++) {
							estr += "<tr>";
							...
							estr += "<td>" + m['e'][i]['oldvalue'] + "</td>";
							estr += "<td>" + m['e'][i]['newvalue'] + "</td>";
...
function editpoints(uid,gid) {
	console.log('editpoints',gid);
	$.ajax({
		type: 'POST',
		url: mulu + 'suser.php',
		cache: false,
		dataType: "json",
		data: 'xtype=editpoints&uid=' + uid+'&gid='+gid,
		success: function(m) {
			//alert(m);
			//alert(1);
			$(".main").hide();
			...
			$(".utb .tuiset").click(function(){
				console.log('gid', $(this).attr('data-gid'));
			...
			$(".setpointssend").click(function() {
				...
				var gid = $(".uid").attr('gid');

				var str = "&uid=" + uid + "&fid=" + fid  + "&gid=" + gid ;
				...
				$.ajax({
					...
					beforeSend:function(){
						...
						//在后台返回success之前显示loading图标 display: block;
					},
					success: function(m) {
						$("#loading").css("display","none");
						if (Number(m) == 1) {
							alert("退水设置已成功保存。");
							console.log("退水设置已成功保存。");
							var gid = $(".uid").attr('gid');
							if(gid == 0){
								$(".main").show();
								$(".utb").hide();
								changeh();
								window.scroll(0, 0);
								window.location.href = mulu+"suser.php?xtype=show&layer=";
							}
	...
	$("#yingdeny").blur(function() {
		...
		var yingdeny = $("#yingdeny").val();
				...
				$(".addtb2 tbody tr").each(function(i) {
						...
						var upmaxzc = $(this).find(".upzc").attr('maxzc');
    				 	var zzc = parseInt(upzc)+parseInt(zcmin);
        				var zzc2 = parseInt(upzc)+parseInt(zc);
        				var upminzc = $(this).find(".upzc").attr('minzc');
        				if(parseInt(upzc) < parseInt(upminzc)){
        				     error =  1;
        				    alert("本级占成不得低于"+upminzc+"！");
        				...
        				console.log(zcmin);
        				console.log(zzc2);
        				if((zzc > upmaxzc)||(zzc2 > upmaxzc)){
        			    	// alert("上级占层加本级占层不得高于"+upmaxzc+"！");
        			     //   error =  1;
        			     //    return false;
