				...
				$.ajax({
					...
					success: function(m) {
						if (Number(m) == 1) {
							...
							$(".main").show();
							$(".utb").hide();
							document.getElementById('frame').window.location.href = mulu+"suser.php?xtype=show&layer=";
						}else{
							alert(m);
						}
					}
...
function addfunc(types) {
	...
	$(".utb input.add").click(function() {
		...
		str += "&tname=" + tname + "&qq=" + qq + "&tel=" + tel + "&sex=" + sex + "&birthday=" + birthday + "&bz=" + bz + "&shengshi=" + shengshi + "&street=" + street 
		+ "&shr=" + shr + "&cssz=" + cssz + "&mgid=" + mgid + "&yingdeny=" + yingdeny;
		var gamecs = "{";
		j = 0;
		$(".addtb2 tbody tr").each(function(i) {
			var typeid = $(this).find("th:eq(0)").attr("typeid");
