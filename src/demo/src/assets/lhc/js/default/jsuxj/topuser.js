function myready(){
	...
	/*
    console.log('myready---',$("#lotterys a.selected").attr('gid'));
	var lhc_gid = new Array(100,200,300,350);
	var gid = Number($(this).attr('gid') );
	console.log('gid---',gid);
	if ($.inArray(gid ,lhc_gid) != -1) {
		window.location.href =  "/member/index?gids="+$(this).attr('gid');
	}else{
		frame.window.location.href = mulu + "make/index?xtype=show&gids="+$(this).attr('gid');
	}*/

	console.log("=======");
	console.log(mulu + "index/makes_lhc?xtype=show&gids="+$("#lotterys a.selected").attr('gid'));
	console.log("=======");
	frame.window.location.href = mulu + "index/makes_lhc?xtype=show&gids="+$("#lotterys a.selected").attr('gid');
	//$(".ui-dialog" ).draggable();
	$(".sendtb").draggable();
	...
	$(".menu2 a").click(function(){
		...
		$(".sub div a").click(function(){
			...
			console.log('.menu2 a click',$("#lotterys a.selected").attr('gid'));
			frame.window.location.href = mulu + "make/index?xtype=show&gids="+$("#lotterys a.selected").attr('gid')+"&menu="+index;
		});
    ...
    $(".moregames a").click(function(){
	    ...
	    $("#result_balls").html('');
		console.log('104 .moregames a click',$(this).attr('gid'));
		 frame.window.location.href = mulu + "make/index?xtype=show&gids="+$(this).attr('gid');
	});	
    ...
    $(".moregame a").click(function(){
	    ...
	    $("#result_info").attr('v',0);
	    $("#result_balls").html('');
		console.log('158 .moregame a click',$(this).attr('gid'));
		 frame.window.location.href = mulu + "make/index?xtype=show&gids="+$(this).attr('gid');
	});	
	    ...
	    $.ajax({
		    ...
		    success: function(m) {
		    	...
		    	$(".moregame a").click(function(){
	                ...
	                $("#result_info").attr('v',0);
	                $("#result_balls").html('');
					console.log('248 .moregame a click',$(this).attr('gid'));
		            frame.window.location.href = mulu + "make/index?xtype=show&gids="+$(this).attr('gid');
	            });	
...
function lotterysclick(){
	$("#lotterys a.g").unbind('click');
	 $("#lotterys a.g").click(function(){
         ...
         $("#result_info div:eq(1)").html('');
	     $("#result_balls").html('');
		 console.log('281 #lotterys a.g click',$(this).attr('gid'));
		 var lhc_gid = new Array(100,200,300,350);
		 var gid = Number($(this).attr('gid') );
		 if ($.inArray(gid ,lhc_gid) != -1) {
			if($('.issixLhcgid').val() == 1){
				frame.window.location.href = mulu + "make/index?xtype=show&gids="+$(this).attr('gid');
			}else{
				window.location.href =  "/member/index?gids="+$(this).attr('gid');
			}

			 // window.location.href = mulu + "make/index?xtype=show&gids="+$(this).attr('gid');
		 }else{
			 frame.window.location.href = mulu + "make/index?xtype=show&gids="+$(this).attr('gid');
		 }
