function exe() {
	...
	var je = 0;
	for (j = 0; j < i; j++) {
		console.log(play[j]['peilv1']);
		if(play[j]['peilv1'] == undefined) {
			console.log('320',play[j]['peilv1']);
			continue ;
		}
		...
		if (bname == '合肖' | bname == '連碼' | bname == '不中' | bname == '生肖連' | bname == '尾數連') {
			str += play[j]['name'] + '&nbsp;<label class=red>' + play[j]['con'].join('-') + "</label>"
		} else if (bname == '過關' | bname == '过关') {
			str += play[j]['classx'] + '&nbsp;<label class=red>' + play[j]['con'] + "</label>"
		} else {
			str +=  play[j]['classx'] + '&nbsp;<label class=red>' + play[j]['name'] + "</label>"
		}
		str += "@" + play[j]['peilv1'];
		...
		str += "</td>";
		str += "<td class='amount'>" + play[j]['je'] + "<input  style='display:none;' value='" + play[j]['je'] + "' /> </td>";
		str += "<td class='zt '  style='display:none;' ><input type='checkbox' checked></td>";
		str += "</tr>";
		je += Number(play[j]['je'])
	}
	var obj = $("#betsBox",parent.document);
	// obj.css("height","auto");
	obj.find("#betlist").html(str);
	obj.find("#bcount").attr('v',i);
	obj.find("#bcount").html(i);
	obj.find("#btotal").html(je);
	...
	$.ajax({
		...
		success: function(m) {
			...
			if(bname == '番摊' ){
				...
				$('.head').html( "<tr class='head'><th > "+$('.lottery_info').html()+" </th></tr>");
				var  str = "<tr>";
				var strzh = "<tr>";
				var strzh = "";
				var strtmp = '';
				...
				if($(this).hasClass("selected")){
					$(".make input:text").each(function(i) {
                        ...
                        if(Number(pname)==Number(val)){
							...
							$(".make td.i" + pid+" input.ba").val('');
							$(".make td.p" + pid+" input.ba").val('');
							$(".make th.m" + pid+" input.ba").val('');
					...
					$(".make input:text").each(function(i) {
                        ...
                        if(Number(pname)==Number(val)){
							...
							$(".make td.i" + pid+" input.ba").val(zje);
							$(".make td.p" + pid+" input.ba").val(zje);
							$(".make th.m" + pid+" input.ba").val(zje);
