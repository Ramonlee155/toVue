...
function libs(stype, gpl) {
	...
	$.ajax({
		...
		success: function(m) {
			...
			if (gpl == 1) {
				for (i = 0; i < ml; i++) {
					$(".p" + m[i]['pid']).find("label.peilv1").html(rpeilv(m[i]['peilv1'].toFixed(4), m[i]['ifok']))
				}
				return false
			}
            $(".make").html('');
			if(bname == '番摊'){
				...
				for (j = 0; j < ck; j++) {
					...
					if (i == 88) {
						...
						str1 += "<td class='odds p p" + m[i]['pid'] + "' pid='" + m[i]['pid'] + "' mname='" + m[i]['name'] + "' maxje='" + m[i]['maxje'] + "' minje='" + m[i]['minje'] + "'><label class='peilv1'>" + rpeilv(m[i]['peilv1'].toFixed(4), m[i]['ifok']) + "</label></td>";
...
function rhtmls(arr, q) {
	...
	strc += "</th>";
	strc += "<td class='odds p p" + arr['pid'] + "' pid='" + arr['pid'] + "' sname='" + arr['sname'] + "' cname='" + arr['cname'] + "' mname='" + arr['name'] + "' maxje='" + arr['maxje'] + "' minje='" + arr['minje'] + "'><label class='peilv1'>" + rpeilv(arr['peilv1'].toFixed(4), arr['ifok']) + "</label></td>";
