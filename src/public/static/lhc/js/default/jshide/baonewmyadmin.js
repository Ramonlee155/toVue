	...
	$.ajax({
		...
		success: function(m) {
			...
			for (i = 0; i < ml; i++) {
				...
				str += "<td>" + m[i]['ushui'] + "</td>";
				str += "<td  class='result color'>" + m[i]['uyk'] + "</td>";
...
function agentnew() {
    ...
    progressBar.style.width = '0%'; // 设置初始宽度为100%
    setTimeout(function() {
      progressBar.style.width = '100%'; // 10秒后将宽度设置为0%
    }, 100);
	...
	$.ajax({
		...
		success: function(m) {
			...
			for (i = 0; i < ml; i++) {
				...
				str += "<td>" + m[i]['upje'] + "</td>";
				if(jsstatus=='true'){
				   str += "<td   class='result color ff' >" + getResult(Number(m[i]['uzhong']) - Number(m[i]['uje']), 1) + "</td>";
				}else{
				   str += "<td>" +0 + "</td>";
				}
				str += "<td>" + m[i]['ushui'] + "</td>";
				str += "<td class='result color ff'>" + m[i]['uyk'] + "</td>";
		...
		success: function(m) {
			...
			for (j = 0; j < ml; j++) {
				...
				for (i = 0; i < bl; i++) {
					...
					str += "<td class='result color ff'>" + bao[i]['uyk'] + "</td>";
